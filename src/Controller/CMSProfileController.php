...
namespace Novatio\SecurityHardener\Controller;

use Novatio\SecurityHardener\Data\BackupToken;
use Novatio\SecurityHardener\Extensions\TwoFactorAuthMemberExtension;
...
class CMSProfileController extends \SilverStripe\Admin\CMSProfileController
{
    ...
    public function getEditForm($id = null, $fields = null)
    {
        ...
        if (!$member || !SiteConfig::current_site_config()->EnableTwoFactorAuth) {
            return $form;
        }
        ...
        if(TwoFactorAuthMemberExtension::validated_activation_mode()){
    ...
    private function addTokenInfo(FieldList &$fields)
    {
        ...
        if ($member->Has2FA) {
            ...
            $two_factor_fields[] = ToggleCompositeField::create(SecurityToken::class, 'Security token',
    ...
    public function load_token_data($request)
    {
        ...
        if(TwoFactorAuthMemberExtension::validated_activation_mode() && !$member->Has2FA){
            // set new secret/token on member
            $member->generateTOTPToken();
            $member->write();
        }
    ...
    public function regenerate_backup_tokens($data, $form)
    {
        ...
        foreach (range(1, Config::inst()->get(BackupToken::class, 'num_backup_tokens')) as $i) {
     ...
     */
    public function handleRequest(HTTPRequest $request, $model = null)
    {
        $response = parent::handleRequest($request, $model);

        $response->addHeader('X-Controller', Convert::raw2htmlname(str_replace('\\', '-', static::class)));
